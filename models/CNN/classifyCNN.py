...
def loadData(matrixfilename,data_max,classificationfilename,classificationlevel):
        ...
        X=np.zeros((len(testvectors),4096))
        testseqIDs=[]
        taxa=[]
        tic = time.time()
        for i_v, vector in enumerate(testvectors):
                if i_v%5000==0:
                        tac = time.time()
                        print(i_v, tac-tic)
                        tic = tac
                ...
                taxonname=""
                if seqid in classificationdict: #seqids:
                        #taxonname= classification[seqids.index(seqid)]
                        taxonname= classificationdict[seqid]
                taxa.append(taxonname)
                #try:
                X[i_v,:] = [float(e) for e in elements[1:]]
                #X.append(elements[1:])
                #X.append([int(e) for e in elements[1:]])
                #except:
                #      print('foo')  
                #print(elements[1:])
        ...
        for i_c, c in enumerate(classes):
                classesdict[c]=i_c
        tic = time.time()
        for i_t, taxonname in enumerate(taxa):
                if i_t%5000==0:
                        tac = time.time()
                        print(i_t, tac-tic)
                        tic = tac
                if taxonname in classesdict:
                        Y.append(classesdict[taxonname])
...
if __name__ == "__main__":
        ...
        if not os.path.isfile(matrixfilename):
                print('Matrix file', matrixfilename, 'not found, running fasta2matrix with', testfastafilename)
        ...
        #load model
        print('Loading Keras model:', classifiername)
        model = load_model(classifiername, compile=False)
